...
namespace Xaminals.Views
{
    ...
    public partial class GoodTeaDetailPage : ContentPage
    {
        public GoodTeaDetailPage()
        {
            InitializeComponent();
        }
        public string Name
        ...
        void LoadDrink(string name)
        {
            try
            {
                Drink drink = GoodTeaData.GoodTea.FirstOrDefault(a => a.Name == name);
        ...
        private void buy(object sender, EventArgs e)
        {
            if ((sizem.BackgroundColor == Color.FromHex("#dddddd") & 
                ...
                quantity.Text=="0")
            {
                DisplayAlert("警告", "大小，冰塊，甜度，數量請填妥", "確認");
            }
            else
            {
                if (sizel.BackgroundColor == Color.FromHex("#0000ff"))
